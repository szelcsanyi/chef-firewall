unless IPFinder.find(node, :public_ipv4).empty?
  ...
  firewall_rule 'log bad flagged packages' do
    jump 'LOG --log-prefix "BADFLAGS: "'
    chain 'BADFLAGS'
    enabled false
  end

  firewall_rule 'drop bad flagged packages' do
    jump 'DROP'
    chain 'BADFLAGS'
  end
  ...
  firewall_rule 'Example whitelist' do
    rule '-s 1.2.3.4'
    jump 'ACCEPT'
    chain 'WHITELIST'
    enabled false
  ...
  firewall_rule 'Example blacklist' do
    rule '-s 1.2.3.4'
    jump 'DROP'
    chain 'BLACKLIST'
    enabled false
  ...
  firewall_rule 'Allow icmp time exceeded' do
  ...
  firewall_rule 'Allow icmp fragmentation needed' do
  ...
  firewall_rule 'Check for tcp flags' do
    jump 'TCP_FLAGS'
    chain 'PUBLIC'
  end

  # reply something for traceroute probes
  firewall_rule 'Traceroute udp packets' do
  ...
  IPFinder.find(node, :private_ipv4).collect { |addr| addr[:iface].split(':')[0] }.uniq.each do |iface|
  ...
  IPFinder.find(node, :public_ipv4).collect { |addr| addr[:iface].split(':')[0] }.uniq.each do |iface|
  ...
  firewall_rule 'Allow established connections in forward' do
    rule '-m state --state RELATED,ESTABLISHED'
    jump 'ACCEPT'
    chain 'FORWARD'
  end
